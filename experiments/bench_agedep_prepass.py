#!/usr/bin/env python
"""Archived age-dependent pre-pass ablation.

This script targeted the former single fused-step interface and cannot measure
the current globally ordered rate/reduction/transition engine. It exits before
constructing a benchmark so stale manuscript tables cannot be regenerated by
accident. Use ``benchmark_acceptance.py`` and profile the production path.

Historically, two implementations were compared, each
...
def main():
    raise SystemExit(
        "bench_agedep_prepass.py is archived: its subclass targets the removed "
        "single-step fused interface. Use experiments/benchmark_acceptance.py "
        "and remeasure the current split pipeline."
    )


def _archived_main():
    """Retain the historical implementation for auditability only."""
